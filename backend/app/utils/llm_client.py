...
class LLMClient:
    ...
    def _is_nim(self) -> bool:
        """Check if we're talking to NVIDIA NIM."""
        return 'nvidia' in (self.base_url or '').lower() or 'integrate.api' in (self.base_url or '').lower()
    ...
    def chat(
        ...
        if response_format and not self._is_nim():
