...
logger = logging.getLogger('nemofish.embedding')
...
class EmbeddingService:
    ...
    def __init__(
        ...
        # Detect NIM/OpenAI vs Ollama by checking if base_url contains 'nvidia' or 'openai'
        self._is_openai_format = any(x in self.base_url.lower() for x in ['nvidia', 'openai', 'integrate.api'])
    ...
    def embed_batch(self, texts: List[str], batch_size: int = 32) -> List[List[float]]:
        ...
        for i, text in enumerate(texts):
            ...
            if text in self._cache:
                ...
                results[i] = [0.0] * (1024 if self._is_openai_format else 768)
    ...
    def _request_embeddings(self, texts: List[str]) -> List[List[float]]:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if self._is_openai_format:
                    # OpenAI/NIM format: {"data": [{"embedding": [...]}]}
