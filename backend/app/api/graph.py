...
from .payment import verify_payment, _increment_wallet_usage
...
def generate_ontology():
    ...
    try:
        ...
        # x402 payment gate: verify payment or free tier before processing
        wallet_address = request.headers.get('X-Wallet-Address', 'anonymous')
        authorized, reason = verify_payment(wallet_address)
        if not authorized:
            return jsonify({
                "success": False,
                "error": "Payment required. $2 USDC per prediction via x402.",
        ...
        # Record prediction usage for x402 billing
